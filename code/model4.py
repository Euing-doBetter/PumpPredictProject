...
# 테스트 파일 경로 (사용자가 지정할 수 있도록 변수로 정의)
test_flow_10min_x_path ='C:/Users/euing/Desktop/펌프장 유입량 예측 AI모델 입력자료(수정)/Test/독립변수/데이터전처리2/Test_Flow_df_10min_X.csv'  # 사용자가 지정할 테스트 파일 경로
test_rain_abs_path = 'C:/Users/euing/Desktop/펌프장 유입량 예측 AI모델 입력자료(수정)/Test/독립변수/데이터전처리2/Rain_df(3hr)_X(전처리파일일).csv'      # 사용자가 지정할 테스트 파일 경로
test_suwi_merged_path = 'C:/Users/euing/Desktop/펌프장 유입량 예측 AI모델 입력자료(수정)/Test/독립변수/데이터전처리2/합쳐진모니터링수위데이터_18행.csv'   # 사용자가 지정할 테스트 파일 경로
test_flow_10min_y_path = 'C:/Users/euing/Desktop/펌프장 유입량 예측 AI모델 입력자료(수정)/Test/종속변수(정답)/Test_Flow_df(3hr)_Y.csv'  # 사용자가 지정할 테스트 파일 경로

# 결과를 저장할 디렉토리 생성
results_dir = 'C:/Users/euing/Desktop/펌프장 유입량 예측 AI모델 입력자료(수정)/결과_BiLSTM2' # BiLSTM 결과 저장 폴더
...
if df_flow_x is None or df_rain is None or df_suwi is None or df_flow_y is None:
    ...
    # 3. 데이터 분할 (학습 세트와 검증 세트)
    # 샘플 수 891개
    train_size = int(num_samples_actual * 0.8) # 891개 중 80% = 712개 학습, 179개 검증
    if train_size == 0: train_size = 1 # 최소 1개 샘플은 학습
    X_train, X_val = X_data[0:train_size, :, :], X_data[train_size:num_samples_actual, :, :]
    y_train, y_val = y_scaled[0:train_size, :, :], y_scaled[train_size:num_samples_actual, :, :]

    print(f"\n학습 데이터 shape (X_train, y_train): {X_train.shape}, {y_train.shape}") # (712, 18, 6), (712, 18, 1)
    print(f"검증 데이터 shape (X_val, y_val): {X_val.shape}, {y_val.shape}") # (179, 18, 6), (179, 18, 1)
    ...
    history = model.fit(
        ...
        epochs=100,
        batch_size=32,
        validation_data=(X_val, y_val),  # 검증 데이터 직접 지정
    ...
    print(f"{'Metric':<15} | {'Train':<10} | {'Validation':<10}")
    print("-" * 40)
    print(f"{'Loss (MAE)':<15} | {train_loss:<10.4f} | {val_loss:<10.4f}")
    print(f"{'MAE':<15} | {train_mae:<10.4f} | {val_mae:<10.4f}")
    print(f"{'MSE':<15} | {train_mse:<10.4f} | {val_mse:<10.4f}")
    ...
    y_test_original = scaler_y.inverse_transform(y_val.reshape(-1, 1)).reshape(y_val.shape)
    ...
    if X_val.shape[0] > 0: # 테스트 샘플이 있을 경우
    ...
    # 8. 별도 테스트 파일로 모델 평가 함수
    def evaluate_model_with_test_files(model, scaler_flow, scaler_rain, scaler_suwi, scaler_y, 
                                      test_flow_path, test_rain_path, test_suwi_path, test_y_path):
        """
        별도의 테스트 파일을 사용하여 모델을 평가하는 함수
        
        Parameters:
        -----------
        model : tf.keras.Model
            학습된 모델
        scaler_flow, scaler_rain, scaler_suwi, scaler_y : sklearn.preprocessing.MinMaxScaler
            각 데이터에 대한 스케일러
        test_flow_path, test_rain_path, test_suwi_path, test_y_path : str
            테스트 파일 경로
        ...
        # 테스트 파일 로드
        df_test_flow_x = load_csv_file(test_flow_path)
        df_test_rain = load_csv_file(test_rain_path)
        df_test_suwi = load_csv_file(test_suwi_path)
        df_test_flow_y = load_csv_file(test_y_path)
        
        if df_test_flow_x is None or df_test_rain is None or df_test_suwi is None or df_test_flow_y is None:
            print("테스트 파일 로드에 실패했습니다. 경로 및 파일 내용을 확인해주세요.")
            return None
        
        print("\n테스트 파일 로드 성공.")
        print(f"Test Flow_X shape: {df_test_flow_x.shape}")
        print(f"Test Rain shape: {df_test_rain.shape}")
        print(f"Test Suwi shape: {df_test_suwi.shape}")
        print(f"Test Flow_Y shape: {df_test_flow_y.shape}")
        
        # 테스트 데이터 재구성 (학습 데이터와 동일한 방식)
        # Flow_X, Rain, Flow_Y 데이터를 Transpose하여 (샘플 수, 시퀀스 길이) 형태로 만듦
        df_test_flow_x_T = df_test_flow_x.T
        df_test_rain_T = df_test_rain.T
        df_test_flow_y_T = df_test_flow_y.T
        
        # 테스트 데이터 스케일링
        df_test_flow_x_T_scaled = scaler_flow.transform(df_test_flow_x_T)
        df_test_rain_T_scaled = scaler_rain.transform(df_test_rain_T)
        df_test_suwi_scaled = scaler_suwi.transform(df_test_suwi)
        
        # Suwi 데이터 재구성 (학습 데이터와 동일한 방식)
        test_suwi_repeated = np.tile(df_test_suwi_scaled[:df_test_flow_x.shape[0], :], (df_test_flow_x_T.shape[0], 1, 1))
        
        # 독립 변수 데이터 재구성
        df_test_flow_x_T_scaled_expanded = np.expand_dims(df_test_flow_x_T_scaled, axis=-1)
        df_test_rain_T_scaled_expanded = np.expand_dims(df_test_rain_T_scaled, axis=-1)
        
        X_test_data = np.concatenate([df_test_flow_x_T_scaled_expanded, df_test_rain_T_scaled_expanded, test_suwi_repeated], axis=-1)
        
        # 종속 변수 데이터 재구성
        y_test_data = np.expand_dims(df_test_flow_y_T.values, axis=-1)
        y_test_scaled = scaler_y.transform(y_test_data.reshape(-1, 1)).reshape(y_test_data.shape)
        
        print(f"\n테스트 데이터 재구성 완료.")
        print(f"재구성된 테스트 독립 변수 shape: {X_test_data.shape}")
        print(f"재구성된 테스트 종속 변수 shape: {y_test_scaled.shape}")
        
        # 모델 평가
        test_loss, test_mae, test_mse = model.evaluate(X_test_data, y_test_scaled, verbose=0)
        
        # 예측 결과
        y_pred_scaled = model.predict(X_test_data)
        
        # 예측 결과를 원래 스케일로 되돌립니다.
        y_test_original = scaler_y.inverse_transform(y_test_scaled.reshape(-1, 1)).reshape(y_test_scaled.shape)
        y_pred_original = scaler_y.inverse_transform(y_pred_scaled.reshape(-1, 1)).reshape(y_pred_scaled.shape)
        ...
        # 결과 출력
        print("\n--- 테스트 파일 평가 결과 (Original Scale) ---")
        print(f"{'Metric':<15} | {'Value':<10}")
        print("-" * 30)
        print(f"{'MAE':<15} | {original_mae:<10.4f}")
        print(f"{'MSE':<15} | {original_mse:<10.4f}")
        print(f"{'R2':<15} | {original_r2:<10.4f}")
        print(f"{'ME':<15} | {hydrological_metrics['ME']:<10.4f}")
        print(f"{'RMSE':<15} | {hydrological_metrics['RMSE']:<10.4f}")
        print(f"{'PBIAS':<15} | {hydrological_metrics['PBIAS']:<10.4f}")
        print(f"{'NSE':<15} | {hydrological_metrics['NSE']:<10.4f}")
        print(f"{'VE':<15} | {hydrological_metrics['VE']:<10.4f}")
        print(f"{'KGE':<15} | {hydrological_metrics['KGE']:<10.4f}")
        print("-" * 30)
        ...
        # CSV 파일 저장 경로
        test_csv_save_path = os.path.join(results_dir, "test_evaluation_results_BiLSTM.csv")
        metrics_df.to_csv(test_csv_save_path, index=False)
        
        print(f"\n테스트 평가 지표 결과가 '{test_csv_save_path}' 파일로 저장되었습니다.")
        
        # 결과 시계열 그래프 (실제값 vs 예측값)
        if X_test_data.shape[0] > 0:  # 테스트 샘플이 있을 경우
            sample_to_plot = 0  # 첫 번째 테스트 샘플
            ...
            plt.title(f'Observed vs BiLSTM Prediction Time Series (Test Sample {sample_to_plot+1})')
            plt.xlabel('Future Time Step (10-min intervals)')
            plt.ylabel('Flow (Original Scale)')
            plt.legend()
            plt.grid(True)
            
            test_timeseries_graph_path = os.path.join(results_dir, 'test_observed_vs_predicted_timeseries_BiLSTM.png')
            plt.savefig(test_timeseries_graph_path)
            print(f"테스트 결과 시계열 그래프가 '{test_timeseries_graph_path}' 파일로 저장되었습니다.")
        else:
            print("테스트 샘플이 없어 시계열 그래프를 그릴 수 없습니다.")
        
        return {
            'scaled_metrics': {'loss': test_loss, 'mae': test_mae, 'mse': test_mse},
            'original_metrics': {
                'mae': original_mae, 'mse': original_mse, 'r2': original_r2,
                'me': hydrological_metrics['ME'], 'rmse': hydrological_metrics['RMSE'],
                'pbias': hydrological_metrics['PBIAS'], 'nse': hydrological_metrics['NSE'],
                've': hydrological_metrics['VE'], 'kge': hydrological_metrics['KGE']
            }
        }

    # 9. 모델 저장
    model_save_path = os.path.join(results_dir, "bilstm_model.h5")
    model.save(model_save_path)
    print(f"\n학습된 모델이 '{model_save_path}' 파일로 저장되었습니다.")

    # 10. 테스트 파일 경로가 지정된 경우 테스트 실행
    if test_flow_10min_x_path and test_rain_abs_path and test_suwi_merged_path and test_flow_10min_y_path:
        print("\n테스트 파일 경로가 지정되어 테스트를 실행합니다.")
        test_results = evaluate_model_with_test_files(
            model, scaler_flow, scaler_rain, scaler_suwi, scaler_y,
            test_flow_10min_x_path, test_rain_abs_path, test_suwi_merged_path, test_flow_10min_y_path
        )
        if test_results:
            print("\n테스트 완료.")
    else:
        print("\n테스트 파일 경로가 지정되지 않아 테스트를 실행하지 않습니다.")
        print("테스트를 실행하려면 다음 변수에 테스트 파일 경로를 지정하세요:")
        print("  - test_flow_10min_x_path")
        print("  - test_rain_abs_path")
        print("  - test_suwi_merged_path")
        print("  - test_flow_10min_y_path")
        print("\n예시:")
        print("test_flow_10min_x_path = 'C:/path/to/test/Flow_df_10min.csv'")
        print("test_rain_abs_path = 'C:/path/to/test/Rain_df(3hr)_X.csv'")
        print("test_suwi_merged_path = 'C:/path/to/test/합쳐진모니터링수위데이터.csv'")
        print("test_flow_10min_y_path = 'C:/path/to/test/Flow_df_10min_Y.csv'")
        print("\n그 후 스크립트를 다시 실행하거나 evaluate_model_with_test_files 함수를 직접 호출하세요.")
